...
int main()
{
    ...
    */
    ...
    sqlite3* db = NULL;
    sqlite3_stmt* pStmt;
    const char* path = "D:\\sqlite\\tp.sqlite3";
    int ret;
...
void insert(sqlite3* db) {
    ...
    */
    ...
    for (nowTrace = 1; nowTrace <= maxTrace; ++nowTrace) {
        while (isContinue) {
            string board_string;
            ...
            for (int i = 0; i < board_size; ++i) {
                board_string += to_string(board[i]);
            }
            sqlite3_bind_text(pStmt, 3, board_string.c_str(), board_size, SQLITE_TRANSIENT);
            while (sqlite3_step(pStmt) == SQLITE_BUSY) {}
            sqlite3_reset(pStmt);
...
void select(sqlite3* db) {
    ...
    sqlite3_prepare_v2(
        db,
        "SELECT id, trace, board FROM pattern WHERE id BETWEEN ? AND ?;",
        -1,
        &pStmt,
        nullptr
    );
    sqlite3_bind_int64(pStmt, 1, 1);
    sqlite3_bind_int64(pStmt, 2, 10);

    while (sqlite3_step(pStmt) == SQLITE_ROW) {
        int64_t id = sqlite3_column_int64(pStmt, 0);
        int64_t trace = sqlite3_column_int64(pStmt, 1);
        const unsigned char* board = sqlite3_column_text(pStmt, 2);
        string str_board = (const char*)board;

        cout << "id    : " << id << endl;
        cout << "trace : " << trace << endl;
        cout << "board : " << str_board << endl;
    }
    sqlite3_reset(pStmt);
    sqlite3_clear_bindings(pStmt);
    sqlite3_finalize(pStmt);
